...
function initializeClientInquiryForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            alert('There was an error submitting your form. Please try again or call us directly at (401) 555-0100.');
        }
    });
}
...
function initializeJobApplicationForm() {
    ...
    tradeSelect.addEventListener('change', async () => {
        ...
        screeningQuestions.innerHTML = '<p>Loading screening questions...</p>';
    ...
    form.addEventListener('submit', async (e) => {
        ...
        document.querySelectorAll('[id^="screening_"]').forEach(input => {
            screeningAnswers.push({
                question: input.previousElementSibling ? input.previousElementSibling.textContent : '',
                answer: input.value
            });
        });
        ...
        try {
            ...
            submitBtn.disabled = false;
            submitBtn.textContent = originalText;
            alert('There was an error submitting your application. Please try again or email your resume to jobs@craftconnectstaffing.com');
        }
    });
}
